...
class TestGitRevision < Test::Unit::TestCase
  ...
  def setup
    @revision_with_date = Blastr::GitRevision.new(SAMPLE_SHA, Time.now)
    @revision_without_date = Blastr::GitRevision.new(SAMPLE_SHA)
  end
  ...
  def test_before_comparison_between_dated_revisions
    later_revision = Blastr::GitRevision.new(SAMPLE_SHA, @revision_with_date.date + 1)
  ...
  def test_before_comparison_with_HEAD
    head = Blastr::GitRevision.new("HEAD")
